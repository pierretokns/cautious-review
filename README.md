...
Local-first, keyboard-first Greenhouse review. **0.2.0 is a usable local evidence/queue preview, not a complete ATS replacement. It does not execute live Greenhouse rejections.**
...
[Verified prior art and licenses](docs/PRIOR-ART.md) · [Verification record](docs/BUILD-STATUS.md)
...
CI fans out unit tests, the synthetic browser fixture, and packaging on each pull request/main push. Publication is gated on all three and then rebuilds/packages the same commit in the release job. The GreenMaxing static inspector is an explicit developer tool rather than a release gate. CI is **not** an autonomous coding agent; the separately configured ChatGPT task supplies recurring development work.
