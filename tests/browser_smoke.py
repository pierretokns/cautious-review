...
with sync_playwright() as p, tempfile.TemporaryDirectory() as profile, tempfile.TemporaryDirectory() as extension:
    ...
    enable();page.locator('#notes').fill('');page.locator('#notes').press('a');page.wait_for_timeout(100)
    check(rows('reviews')==[],'Typing in input is safe')
    page.locator('#editable').click();page.keyboard.press('r');page.wait_for_timeout(100)
    check(rows('reviews')==[],'Contenteditable is safe')
    page.locator('h1').click();page.keyboard.press('Control+a');page.wait_for_timeout(100)
    check(rows('reviews')==[],'Modified keys are safe')
    page.keyboard.press('Escape');page.locator('h1').click();page.keyboard.press('a');status('ADVANCE queued')
    check(rows('reviews')[0]['decision']=='advance','Advance queued locally')
    page.keyboard.press('1');status('REJECT queued');check(rows('reviews')[0]['decision']=='reject','Reason shortcut works')
    page.keyboard.press('u');status('restored');check(rows('reviews')[0]['decision']=='advance','Undo restores prior decision')
    ...
    panel.locator('#cr-import').set_input_files({'name':'synthetic.json','mimeType':'application/json','buffer':json.dumps(batch).encode()});status('Imported 2')
    ...
    panel.locator('#cr-import').set_input_files({'name':'invalid.json','mimeType':'application/json','buffer':json.dumps(bad).encode()});status('current Greenhouse origin')
    ...
    enable();page.keyboard.press('j');page.wait_for_url('**job_application_id=202');status('Application 202')
    enable();page.keyboard.press('m');status('MAYBE queued')
